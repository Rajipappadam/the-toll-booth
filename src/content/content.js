 ...
 * Listens for SHOW_TOLL from background, fetches a task from the Todoist API
 * (falling back to local storage), then mounts a Shadow DOM toll overlay.
 */
...
async function showToll(site) {
  ...
  // Shadow DOM host — completely isolated from the page's styles
  ...
  // Fetch task (Todoist first, then local)
...
async function completeToll(shadow, task, source, site) {
  ...
  // Remove from local queue (Todoist tasks stay in Todoist)
  if (source === 'local') {
...
// ── Task fetching (Todoist → local fallback) ──────────────────────────────────
...
async function fetchTask() {
  ...
  // Fall back to background's local task picker
