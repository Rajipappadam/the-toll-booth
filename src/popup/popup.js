 ...
 * Handles: task CRUD, site CRUD, settings, Anthropic AI task suggestions.
 */
...
async function init() {
  ...
  // Show AI button only if key is saved
...
async function renderUnlocks() {
  ...
  list.innerHTML = active
    .map(([site, exp]) => {
      const mins = Math.ceil((exp - now) / 60_000);
      return `
...
// ── AI task suggestions (Anthropic API) ───────────────────────────────────────
